...
module renewal_vip_refill_64x64_TB;
    ...
    localparam int INPUT_HEIGHT = 64;
    localparam int KERNEL_WIDTH = 32;
    localparam int KERNEL_HEIGHT = 32;
    localparam int STRIDE_X = 32;
    localparam int STRIDE_Y = 32;
    localparam int OUTPUT_WIDTH = ((INPUT_WIDTH - KERNEL_WIDTH) / STRIDE_X) + 1;
    localparam int OUTPUT_HEIGHT = ((INPUT_HEIGHT - KERNEL_HEIGHT) / STRIDE_Y) + 1;
    ...
    localparam int DATA_CHUNK_CAPACITY = 1024;
    localparam int DATA_CHUNK_LOAD_COUNT = 8;
    localparam int EXPECTED_DATA_BURST_COUNT =
        DATA_CHUNK_LOAD_COUNT * DATA_CHUNK_CAPACITY;
    ...
    localparam string MACLANE_ACTUAL_FILE =
        "renewal_maclane_refill_64x64_actual.txt";
    ...
    int unsigned data_chunk_load_index;
    int unsigned data_chunk_burst_index;
    ...
    task automatic program_64x64_32x32_operation;
        ...
        begin
            ...
            axi_write32(32'h0000_0050, {16'(STRIDE_Y), 16'(STRIDE_X)});
    ...
    task automatic check_maclane_packet(input int unsigned packet);
        ...
        begin
            ...
            data_position = (((output_y * STRIDE_Y) + kernel_y) * INPUT_WIDTH) +
                            (output_x * STRIDE_X) + kernel_x;
            ...
            if ((packet % WEIGHT_POSITION_COUNT) == 0 ||
                packet == (MACLANE_PACKET_COUNT - 1)) begin
                $display("[MACLANE PASS] packet=%0d tag=%0d out=(%0d,%0d) kernel=(%0d,%0d) data_position=%0d",
                         packet, maclane_tag, output_x, output_y,
                         kernel_x, kernel_y, data_position);
            end
        end
    ...
    always @(posedge clk) begin// to avoid non-blocking limit
        int unsigned expected_chunk_base;
        int unsigned expected_chunk_count;
        xil_axi_ulong expected_data_address;
        if (resetn && u_dut.memory_arvalid && u_dut.memory_arready) begin
            ...
            if ((u_dut.memory_araddr >= WEIGHT_DDR_BASE) &&
                (u_dut.memory_araddr <
                 (WEIGHT_DDR_BASE + WEIGHT_POSITION_COUNT * BURST_LEN * 4))) begin
                if (u_dut.memory_araddr !==
                    (WEIGHT_DDR_BASE + weight_burst_count * BURST_LEN * 4)) begin
                    $fatal(1,
                           "Unexpected WEIGHT read: burst=%0d actual=0x%08h",
                           weight_burst_count, u_dut.memory_araddr);
                end
                weight_burst_count = weight_burst_count + 1;
            end else if ((u_dut.memory_araddr >= DATA_DDR_BASE) &&
                         (u_dut.memory_araddr <
                          (DATA_DDR_BASE + DATA_POSITION_COUNT * BURST_LEN * 4))) begin
                // Expected sequence: 0,1,0,1,2,3,2,3.
                expected_chunk_base =
                    (((data_chunk_load_index / 4) * 2) +
                     (data_chunk_load_index % 2)) * DATA_CHUNK_CAPACITY;
                expected_chunk_count = DATA_CHUNK_CAPACITY;
                expected_data_address = DATA_DDR_BASE +
                    ((expected_chunk_base + data_chunk_burst_index) * BURST_LEN * 4);

                if (data_chunk_load_index >= DATA_CHUNK_LOAD_COUNT ||
                    u_dut.memory_araddr !== expected_data_address) begin
                    $fatal(1,
                           "Unexpected DATA refill: chunk=%0d burst=%0d expected=0x%08h actual=0x%08h",
                           data_chunk_load_index, data_chunk_burst_index,
                           expected_data_address, u_dut.memory_araddr);
                end

                if (data_chunk_burst_index == 0) begin
                    $display("[DATA CHUNK LOAD] index=%0d base=%0d count=%0d addr=0x%08h",
                             data_chunk_load_index, expected_chunk_base,
                             expected_chunk_count, u_dut.memory_araddr);
                end

                data_burst_count = data_burst_count + 1;
                data_chunk_burst_index = data_chunk_burst_index + 1;
                if (data_chunk_burst_index == expected_chunk_count) begin
                    data_chunk_burst_index = 0;
                    data_chunk_load_index = data_chunk_load_index + 1;
                end
            end else begin
    ...
    initial begin
        ...
        if (data_burst_count != EXPECTED_DATA_BURST_COUNT ||
            weight_burst_count != WEIGHT_POSITION_COUNT ||
            data_chunk_load_index != DATA_CHUNK_LOAD_COUNT ||
            data_chunk_burst_index != 0) begin
            $fatal(1,
                   "Unexpected burst count: data=%0d/%0d weight=%0d/%0d chunks=%0d/%0d offset=%0d",
                   data_burst_count, EXPECTED_DATA_BURST_COUNT,
                   weight_burst_count, WEIGHT_POSITION_COUNT,
                   data_chunk_load_index, DATA_CHUNK_LOAD_COUNT,
                   data_chunk_burst_index);
        end

        $display("renewal_vip_refill_64x64_TB PASSED: 64x64 DATA, 32x32 WEIGHT, stride 32, and four CBUF chunks verified");
    ...
    initial begin
        ...
        $fatal(1, "renewal_vip_refill_64x64_TB timeout");
    end
